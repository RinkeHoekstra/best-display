...
<?php
  ...
  function aanzetknop(){
   ...
   return "<a class=ord href=\"http://cli.vu//best/display/index.php?".$questr."\">Aanzetten</a>"; 
  }
  function uitzetknop($connum){
   global $conceptset;
   $questr=$conceptset->querystring_without_conceptnr($connum);
   return "<a class=ord href=\"http://cli.vu//best/display/index.php?".$questr."\">Uitzetten</a>"; 
  }
  ...
  function next_paragraphtext($entire, $is_relevant, $annotate){
    if($entire || $is_relevant){
      //      $uits=$uits."MESA:".$this->next_string_edge;
      $bol=TRUE;
	...
	//        $uits=$uits."MES0:".$this->next_string_edge;
        $uits=$uits.( $annotate
                      ? $this->annotate_token($tokenstr,$i)
                      : $tokenstr
                      );
	//        $uits=$uits."MES1:".$this->next_string_edge;
    ...
    //Test:
    //    $uits=$uits."MES1: next_matchpoint_el: ".$this->next_string_edge;
    return $uits;
  }
  ...
  function annotate_token($tk,$toknum){
	...
	$ts=$ts.$value;
      ...
      $ts=$ts."E";
      foreach($endrelevances as $key => $value){
	$ts=$ts.$value;
      };
      $ts=$ts.($changed ? "C" : "U");
      ...
      if(preg_match("/schade/", $tok)) $tok="NNN".$tok."NNN".$this->next_string_edge."NNN";
    };
    return $tok;
  }
  ...
  function spanclassof($relar){
    ...
    foreach($relar as $key => $value){
      if($value>0) $lab=$lab.telwoordvan($value);
    };
    return $lab;
  }
